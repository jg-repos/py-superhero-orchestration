...
locals  {
  ...
  network_name = var.gcp_network_name != null ? var.gcp_network_name : "${local.prefix}-network"
}
...
resource "google_compute_subnetwork" "this" {
  ...
  secondary_ip_range {
    range_name    = "${local.network_name}-svc"
    ip_cidr_range = var.cidr_block_secondary
  }
}
...
resource "databricks_mws_networks" "this" {
  ...
  network_name = "${local.network_name}-workspace-network"
  gcp_network_info {
    ...
    pod_ip_range_name     = "${local.network_name}-pods"
    service_ip_range_name = "${local.network_name}-svc"
  }
}
